...
- Surface views accept `touchMode="pointer" | "direct"`. Direct mode is the default and forwards each event's contact changes as `C2S_SURFACE_TOUCH` for native Wayland multitouch; unsupported servers automatically retain the pointer mapping. Pointer mode is the opt-out compatibility path and maps touch to tap, finger scroll, long-press right-click, and hold-drag. The UI exposes this as **Media → Touch input**.
...
**Size.** A face inlined as base64 is large: 25.4 MB for a four-face PragmataPro Mono. Two things keep that off every page load. The route brotli-compresses the stylesheet for clients that offer `br` (8.7 MB, quality 4 with the largest standard window — see `font_response`), and the frontend keeps what it fetched, in IndexedDB for the CSS and `localStorage` for the advance ratio and the family list (`js/ui/src/fontStore.ts`). A stored face is used for a day — the `max-age` the route serves — then revalidated in the background while the stored copy still draws; a 404 on revalidation drops it, so an uninstalled family stops being served from the client's own cache. The HTTP cache is not enough on its own: a single entry that size is the first thing a browser evicts.
