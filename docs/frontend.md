...
The presenter estimates the PTS→local-clock baseline as the minimum observed `arrival − pts` (the fastest path seen), leaking upward slowly to follow genuine clock drift and clamped on the way down so one fast-arriving burst frame is not mistaken for a faster link. Added latency is the `LATENESS_QUANTILE` (p95) of how late frames landed over the last `LATENESS_WINDOW_MS` (2 s) of stream, hard-capped at `PRESENT_DELAY_MAX_MS` (50 ms) — so a LAN link, where jitter is near zero, buffers almost nothing.
...
The margin is slewed toward that target, not set to it (`MARGIN_GROW_MS` 2 ms/frame up, `MARGIN_SHRINK_MS` 0.25 down). A margin change _is_ a latency change — every future due time shifts with it — so stepping it would inject exactly the discontinuity the scheduler exists to remove. Slewing turns it into a sub-perceptual rate nudge instead, and it still crosses the full range in well under a second. Growth is faster than shrink because being under-buffered judders while being over-buffered only costs a little latency.
