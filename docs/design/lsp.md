...
- Semantic tokens, inlay hints, code lens. Continuous decoration
  ...
  help, originally deferred alongside them, landed with `LSP_BUFFER` —
...
**and** every nonce-bearing request is refused with `PERMISSION`, as KV
and NET do. Dropping them silently left a client that ignores feature
bits — or that raced a mid-session disable — awaiting the one reply per
nonce this family otherwise guarantees. The fire-and-forget opcodes
(`LSP_ACK`, `LSP_CANCEL`, `LSP_BUFFER`) carry no nonce and are dropped,
which is what they get when the family is on and the id is unknown.
...
| C2S       | `0x67` | `LSP_BUFFER`  | `[lsp_id:2][flags:1][path_len:2][path:N][text:LZ4]`                                         |
...
A byte offset landing **inside** a character floors to that character's
start, in every encoding and in both directions. Rounding forward is the
one answer that can move a position onto the far side of the character
the user pointed at, and passing a mid-character offset through is worse
still: a backend negotiating UTF-8 will happily slice there.
...
7 COMPLETION   → COMPLETION records; response INCOMPLETE flag mirrors
                 the server's isIncomplete (retype should re-query)
8 SIGNATURE    → SIGNATURE records, active signature first
```
...
COMPLETION 0x05: [kind:1][item_kind:1][flags:1][line:4][col:4]
               [end_line:4][end_col:4][label_len:2][label:N]
               [insert_len:2][insert:N][detail_len:2][detail:N]
               item_kind: LSP CompletionItemKind (0 unknown); flags:
               bit 0 DEPRECATED, bit 1 SNIPPET (insert is LSP snippet
               syntax — a client without snippet UI degrades it to
               plain text), bit 2 PRESELECT; range = the item's primary
               replace range (zero range → the client picks its own
               word boundary); insert empty → insert the label
SIGNATURE 0x06: [kind:1][flags:1][active_param:2][param_start:2]
               [param_end:2][label_len:2][label:N][doc_len:4][doc:N]
               one per signature, the active signature first (flags
               bit 0 ACTIVE); active_param 0xFFFF = none; param_start/
               param_end = UTF-8 byte range of the active parameter
               within label (0,0 = unknown), transcoded from the
               server's UTF-16 offsets like every position
```
...
The buffer overlay: a client with an open editor streams its live
buffer so language servers see keystrokes, not just saves. `path` is
in emitted (escaped, root-relative) form like query paths; `text` is
the **full** buffer content, LZ4-compressed — full-text sync is what
the engine already speaks to backends, and at editor debounce rates
(~150 ms) delta encoding buys nothing but a version protocol. `flags`
bit 0 `RELEASE` drops the overlay (`text` must be empty).

While a path is overlaid, the overlay is the engine's byte source for
that document: it is admitted to the open set (pinned against LRU
eviction), `didChange`d with overlay bytes, and watcher-driven content
sync for it is suppressed (`didChangeWatchedFiles` still flows —
open-document content is authoritative for every server, the LSP
open-file contract). `DIAG_FILE.hash` and `LOCATION.hash` become the
BLAKE3-128 of the overlay bytes, so the staleness correlation
composes unchanged — a client compares against the hash of what it
last sent. Versions stay engine-minted; the wire carries no version
field, because a connection's messages are ordered and overlay writes
are last-writer-wins state, not a negotiation.

There is deliberately no reply. The message is idempotent absolute
state on an ordered transport; what queries need is _ordering_ —
flush the overlay, then send `LSP_QUERY`, and the engine processes
them in sequence, so a completion always answers against the bytes
the client just typed. Multiple overlays on one path (two editors,
two connections) last-write-win; the overlay is released explicitly,
on `LSP_CLOSE` of the attachment, or on disconnect — every release
path reverts the document to disk truth via one `didChange` of disk
bytes. Overflowing a budget (`BLIT_LSP_BUFFER_MAX`,
`BLIT_LSP_MAX_OVERLAYS`) degrades to exactly that release: the
overlay drops, intelligence falls back to saved state — the
pre-overlay behavior, never an error the editor must handle.
...
`LSP_BUFFER` overlay (§ `LSP_BUFFER`) — and even then versions are
...
holds an `LSP_BUFFER` overlay: an alternate byte source into the same
...
| Buffer overlays per attachment   | 64               | `BLIT_LSP_MAX_OVERLAYS`                       |
| Overlay bytes per document       | 8 MiB            | `BLIT_LSP_BUFFER_MAX`                         |
...
| Diagnostics frozen after         | 600 s            | `BLIT_LSP_DIAGS_COLD_SECS`                    |
| Restarts per backend             | 3/hour           | `BLIT_LSP_MAX_RESTARTS`                       |
| Spawns per daemon                | 30/minute        | `BLIT_LSP_SPAWN_RATE`                         |

The per-response budgets are charged **within** a file as well as
between files. A single pathological one — a generated bundle, or one
syntax error cascading into hundreds of thousands of diagnostics — would
otherwise be appended whole and push the payload past the client's 64 MiB
decompression guard; the client then refuses the update, never acks, the
one-in-flight pacer never clears, and that attachment receives no
diagnostics again for as long as it lives. A file's `DIAG` records are
therefore built before its `DIAG_FILE` record, so the count `n` states
how many actually follow rather than how many existed. 65 535 is that
field's ceiling, not a guess.
...
- Completion trigger characters are not on the wire: the browser
  editor re-queries on its own activation heuristics (word characters
  plus its completion tooltip lifecycle), and signature help triggers
  on `(` and `,` generically. Exposing per-server trigger sets is a
  `SERVER` record field addition if a language ever needs it.
- No `completionItem/resolve` round trip: `COMPLETION` records carry
  label, insert text, and `detail` only — per-item documentation
  popups would need resolve support and a second query surface.
- Snippet insert texts are flagged, never expanded server-side; the
  browser editor degrades them to plain text (placeholders stripped).
- Overlays are engine state: an explicit `LSP_STOP` (or a stopped
  backend respawned by a later query) starts a fresh engine that serves
  disk truth until the editor's next debounced buffer send heals the
  overlay. Persisting overlays across a deliberate stop was rejected —
  `LSP_STOP` is an operator killing a misbehaving backend, and pinned
  megabytes of buffer text surviving it would defeat the point.
...
5. Next query kinds (implementation, call hierarchy) as adoption
   warrants.
6. `LSP_BUFFER` overlays plus query kinds 7–8 (completion, signature
   help), consumed by the browser editor: overlay streamed debounced
   from the CodeMirror buffer and flushed before every query, so
   diagnostics and answers track keystrokes, not saves.

Still gated on the mutation RFC: applying workspace edits, code
actions, formatting. Still gated on browser-editor maturity: semantic
tokens, inlay hints, code lens.
