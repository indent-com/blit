...
- **Not durable unless a file stream is configured.** Ring contents disappear
  ...
  protocol.
- **Not a new authorization boundary.** Event inspection is an administrative
  capability of a direct server connection. It is deliberately unavailable to
  read-only shares and in-process extension endpoints.
...
broadcast channel, and the registry of persistent file tasks. Connection-local
client stream tasks remain in the connection handler, so disconnect cleanup
cannot leave a task holding a dead outbox.
...
mutex used for append and dump.
...
`blit events config` reads the current capacity, retained byte/record counts,
overwrite count, next sequence, and complete 256-bit activation set.
...
neither advertise nor accept the event family.
...
records, followed by new records. A lagged file task inserts synthetic type
`65535` with `[lost:8]`. Client streams report the same condition in an
`EVENTS_STREAM_GAP` envelope.
...
| --- | --- |
| `CONFIG_GET` (1) | empty |
| `CONFIG_SET` (2) | `[size:8][activations:32]` |
| `DUMP` (3) | empty |
...
Stream ids are process-wide and unique. Server-file streams persist after the
requesting client disconnects; client streams stop with their connection and
can be stopped only on that connection.

Server messages start `[0xD0][version:1 = 1][kind:1]`:

| Kind | Body |
| --- | --- |
| `CONFIG` (1) | `[nonce:2][size:8][used:8][records:8][dropped:8][next_sequence:8][activations:32]` |
...
`[stream_id:4][running:1][flags:1][path_len:2][path:N]`. It enumerates the
process-scoped server recordings, including a task that has finished after a
late file error but has not yet been stopped and removed.
...
slow stream can lag and report loss, but never blocks the producer or the ring.
...
| Server-file open fails | Protocol start returns an error; startup configuration reports stderr and records `server.error` when enabled. |
| File write fails after start | That dedicated task terminates without blocking producers or server shutdown. |
...
- Append-mode files can contain consecutive self-describing dump segments.
...
oversized live delivery, graceful file history/live flushing, activation
selectors, strict request/response codecs, and a framed connection that
negotiates the feature, changes configuration, and retrieves a dump. The full
remote and server unit suites and strict Clippy are part of the implementation
verification.
...
lifetime explicit in the grammar.
