...
`content_kind`: `0`/`1` full bytes; `2` a **delta-against-`base`** write
(the same COPY/INSERT instruction stream as fs-watch's content deltas):
the ops apply against the exact bytes `base` names, so a delta requires
a real CAS anchor — `NO_CAS` or a zero base answers `INVALID`. The CAS
check and the base read are one verified read: the target's current
content must hash to `base` (else `CONFLICT`, live hash attached — a
base the server cannot produce is precisely a failed precondition, so a
corrupted apply is impossible by construction), and the ops apply
against those verified bytes. A malformed instruction stream answers
`INVALID`; the ops, the base, and the applied result are each bounded by
`BLIT_FS_WRITE_MAX` (`TOO_LARGE`). A client may always send full
instead — the delta is a bandwidth optimization, never an obligation.
...
(traversal fix, § Path validation). On a `SINGLE` sync
([fs-watch.md](fs-watch.md) § Single-file sync) the namespace holds
exactly one path: writes and ops address the empty string — the root
