...
where
    ...
    B::Handle: Clone,
{
    /// Finish the picture assembled from a complete access unit without
    /// draining the DPB or resetting codec state.
    ///
    /// Packet-oriented callers know the access-unit boundary, while Annex-B
    /// itself does not carry an explicit end marker.  The regular streaming
    /// decoder otherwise holds this picture until the first VCL NAL of the
    /// next access unit, adding one packet of latency and producing no output
    /// for an isolated recovery access unit.
    pub fn end_access_unit(&mut self) -> Result<(), DecodeError> {
