...
import { fsCompress, fsCompressLiteral, fsDecompress } from "./fs.js";
 ...
 *  paths); while overlaid, the server engine's byte source for the
 *  document is the overlay, not disk (docs/design/lsp.md "LSP_BUFFER").
 *  Flags bit 0 {@link LSP_BUFFER_RELEASE} drops the overlay (`text`
 *  empty). No reply: idempotent last-writer-wins state on an ordered
 *  transport. */
export const C2S_LSP_BUFFER = 0x67;
...
export const LSP_OPEN_FROM_PTY = 1 << 2;
...
/** → `COMPLETION` records; the response's {@link LSP_RESP_INCOMPLETE}
 *  flag mirrors the server's `isIncomplete` (retype should re-query). */
export const LSP_QUERY_COMPLETION = 7;
/** → `SIGNATURE` records, active signature first. */
export const LSP_QUERY_SIGNATURE = 8;
 ...
 *  be empty). */
export const LSP_BUFFER_RELEASE = 1 << 0;
...
export const LSP_CAP_COMPLETION = 1 << 6;
export const LSP_CAP_SIGNATURE = 1 << 7;
...
export const LSP_SIGNATURE_ACTIVE = 1 << 0;

/** `SIGNATURE.activeParam` value when no parameter is active. */
export const LSP_SIGNATURE_NO_PARAM = 0xffff;
...
export const LSP_QUERY_RECORD_COMPLETION = 0x05;
export const LSP_QUERY_RECORD_SIGNATURE = 0x06;
...
export function msgLspOpen(
  ...
  pushU16(buf, nonce);
  buf.push(hasSrc ? flags | LSP_OPEN_FROM_PTY : flags);
 ...
 *  LZ4-compressed on the wire. A {@link LSP_BUFFER_RELEASE} must carry
 *  empty `text`. */
export function msgLspBuffer(
  lspId: number,
  flags: number,
  path: string,
  text: Uint8Array,
): Uint8Array {
  const buf: number[] = [C2S_LSP_BUFFER];
  pushU16(buf, lspId);
  buf.push(flags);
  pushStr(buf, path);
  const head = new Uint8Array(buf);
  const compressed = fsCompress(text);
  const msg = new Uint8Array(head.length + compressed.length);
  msg.set(head, 0);
  msg.set(compressed, head.length);
  return msg;
}

/** Parse `C2S_LSP_BUFFER` into `[lspId, flags, path, text]` with the
 *  text decompressed (tests and mocks; the server is the consumer). */
export function parseLspBuffer(
  msg: Uint8Array,
): [number, number, string, Uint8Array] | null {
  if (msg.length < 6 || msg[0] !== C2S_LSP_BUFFER) return null;
  const c = new Cursor(msg.subarray(1));
  const lspId = c.u16();
  const flags = c.u8();
  const path = c.str();
  const text = fsDecompress(c.rest());
  if (!c.ok || text === null) return null;
  return [lspId, flags, path, text];
}
...
export type LspQueryRecord =
   ...
   *  One signature, the active one first ({@link LSP_SIGNATURE_ACTIVE});
   *  `paramStart`/`paramEnd` bound the active parameter within `label`
   *  in UTF-8 bytes (0,0 = unknown). */
  | {
      kind: "signature";
      /** `LSP_SIGNATURE_*` bits. */
      flags: number;
      /** Active parameter index, or {@link LSP_SIGNATURE_NO_PARAM}. */
      activeParam: number;
      paramStart: number;
      paramEnd: number;
      label: string;
...
export function lspQueryRecords(data: Uint8Array): Generator<LspQueryRecord> {
  return records(data, (kind, c) => {
    switch (kind) {
      ...
      case LSP_QUERY_RECORD_COMPLETION:
        return {
          kind: "completion",
          itemKind: c.u8(),
          flags: c.u8(),
      ...
      case LSP_QUERY_RECORD_SIGNATURE:
        return {
          kind: "signature",
          flags: c.u8(),
          activeParam: c.u16(),
...
export function appendLspQueryRecord(
  ...
  switch (record.kind) {
    ...
    case "edit":
      ...
      break;
    case "completion":
      buf.push(LSP_QUERY_RECORD_COMPLETION, record.itemKind, record.flags);
      pushU32(buf, record.line);
      pushU32(buf, record.col);
      pushU32(buf, record.endLine);
      pushU32(buf, record.endCol);
      pushStr(buf, record.label);
      pushStr(buf, record.insert);
      pushStr(buf, record.detail);
      break;
    case "signature":
      buf.push(LSP_QUERY_RECORD_SIGNATURE, record.flags);
      pushU16(buf, record.activeParam);
      pushU16(buf, record.paramStart);
      pushU16(buf, record.paramEnd);
      pushStr(buf, record.label);
      pushText(buf, record.doc);
      break;
  }
  endRecord(buf, start);
}
...
export interface LspOpenOptions {
  ...
  /** Resolve the workspace path from this session's live cwd, so intelligence
   *  follows `cd` (docs/ide.md Decision 3). The session must be on the same
   *  connection. */
...
export interface LspHandle extends ReactiveStore {
  ...
  /** Completion at a position → `completion` records in the server's
   *  ranking order; `incomplete` mirrors `isIncomplete` (retype should
   *  re-query). */
  completion(path: string, line: number, col: number): Promise<LspQueryResult>;
  /** Signature help at a position → `signature` records, active first. */
  signatureHelp(
    path: string,
    line: number,
    col: number,
  ): Promise<LspQueryResult>;
  /** Stream this document's live buffer as its LSP byte source
   *  (docs/design/lsp.md "LSP_BUFFER"). Full text, last-writer-wins,
   *  fire-and-forget: send debounced on edits and flush before queries —
   *  transport ordering makes the query answer against these bytes. */
  buffer(path: string, text: Uint8Array): void;
  /** Drop the overlay: the document reverts to disk truth. (Closing the
   *  attachment or losing the connection releases implicitly.) */
