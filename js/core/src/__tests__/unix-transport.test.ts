...
import { UnixSocketTransport } from "../transports/unix";

// These tests exercise real AF_UNIX sockets on POSIX.  The transport is
// intentionally node-only, so we skip on platforms where `net.connect({
// path })` does not talk to a unix socket (Windows uses named pipes, not
// covered by these tests).
const describeUnix = process.platform === "win32" ? describe.skip : describe;

describeUnix("UnixSocketTransport", () => {
  let tmp: string;
  let sockPath: string;
  let server: Server;
  ...
  it("starts disconnected", () => {
    const t = new UnixSocketTransport(sockPath);
  ...
  it("close() prevents reconnect", async () => {
    const t = new UnixSocketTransport(sockPath, { reconnectDelay: 20 });
    t.connect();
    await waitFor(() => t.status === "connected");
    t.close();
  ...
  it("reports error for unreachable socket path", async () => {
    ...
    const t = new UnixSocketTransport(missing, {
      reconnect: false,
