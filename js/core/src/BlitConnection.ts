...
export class BlitConnection {
  ...
  private readonly sendDesktopMedia = (message: Uint8Array): void => {
    this.transport.send(message);
  };

  private setDesktopMediaTransportStatus(status: ConnectionStatus): void {
  ...
  constructor({
    ...
    this.setDesktopMediaTransportStatus(this.transport.status);
    ...
    if (typeof navigator !== "undefined") {
      const clipboard = navigator.clipboard as
        (Clipboard & { onclipboardchange?: unknown }) | undefined;
    ...
    this.mediaStore.subscribe(() => this.emit());
    this.mprisStore.subscribe(() => this.emit());
  ...
  dispose(): void {
    ...
    this.mediaStore.setSender(null);
    this.mprisStore.reset(connectionError("Connection disposed"));
    this.mprisStore.setSender(null);
  }
  ...
  /** Retire all connection-owned state for an id the server destroyed.
   *  Do not send UNSUBSCRIBE: destruction is already authoritative, and a
   *  delayed unsubscribe could otherwise hit a new surface reusing the id. */
  private retireDestroyedSurface(surfaceId: number): void {
    const sub = this.surfaceSubs.get(surfaceId);
    if (sub && sub.pendingUnsub !== null) {
      clearTimeout(sub.pendingUnsub);
    }
  ...
  /** A malformed sequence desynchronizes the stream, so discard both the
   * allocation and the transport instead of attempting to continue. */
  private abortFragmentReassembly(): void {
    this.resetFragmentReassembly();
    this.transport.close();
  }
  ...
  private ensureFragmentCapacity(required: number): void {
    ...
    while (capacity < required) {
      capacity = Math.min(capacity * 2, MAX_LOGICAL_MESSAGE);
    }
  ...
  private handleMessage = (data: BlitTransportMessage): void => {
    ...
    if (bytes.length === 0) {
      if (this.fragmentCount !== 0) this.abortFragmentReassembly();
      return;
    }
    ...
    if (type === S2C_FRAGMENT) {
      if (bytes.length < 3) {
        this.abortFragmentReassembly();
        return;
      }
      const flags = bytes[1];
      const chunk = bytes.subarray(2);
      if (
        (flags & ~FRAGMENT_FLAG_LAST) !== 0 ||
        chunk.length > MAX_FRAGMENT_CHUNK ||
        this.fragmentCount >= MAX_FRAGMENT_COUNT ||
        this.fragmentBytes > MAX_LOGICAL_MESSAGE - chunk.length
      ) {
        this.abortFragmentReassembly();
        return;
      }
    ...
    // Only audio frames may bypass an in-progress bulk message. Any other
    // opcode makes the logical stream ambiguous and therefore fatal.
    if (this.fragmentCount !== 0 && type !== S2C_AUDIO_FRAME) {
      this.abortFragmentReassembly();
      return;
    }
    ...
    switch (type) {
      ...
      case S2C_MEDIA_CONTROL: {
        const control = parseMediaControl(bytes);
        if (!control) {
          this._logger.warn(`${this.id}: malformed MEDIA_CONTROL`);
          return;
        }
        if (!this.mprisStore.handle(control)) this.mediaStore.handle(control);
        return;
      }
      case S2C_HELLO: {
        if (bytes.length < 7) return;
        ...
        if (features & FEATURE_DESKTOP_MEDIA) {
          this.mprisStore.reconnect();
        }
        return;
      }
      ...
      case S2C_SURFACE_DESTROYED: {
        try {
          ...
          this.retireDestroyedSurface(surfaceId);
  ...
  private handleStatusChange = (status: ConnectionStatus): void => {
    // Media capture and MPRIS actions use the sender's presence as their
    // connection guard. Detach it before resetting state so a stale UI click
    // after transport loss rejects immediately and stops its authorized track
    // instead of starting a lease timer around a silently dropped message.
    this.setDesktopMediaTransportStatus(status);
    ...
    if (
      ...
      this.desktopStore.reset();
      this.mediaStore.reset();
      this.mprisStore.reset(connectionError("Connection lost"));
