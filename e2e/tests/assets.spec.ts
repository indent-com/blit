...
test.describe('Asset serving', () => {
  ...
  test('GET /vt/blit_browser.js returns JavaScript (reverse proxy prefix)', async ({ request }) => {
  ...
  test('GET /anyprefix/snippets/blit-browser-anyhash/inline0.js returns JavaScript', async ({ request }) => {
    const resp = await request.get('/anyprefix/snippets/blit-browser-anyhash/inline0.js');
    expect(resp.status()).toBe(200);
    const ct = resp.headers()['content-type'] ?? '';
    expect(ct.includes('javascript') || ct.includes('ecmascript')).toBeTruthy();
  });
