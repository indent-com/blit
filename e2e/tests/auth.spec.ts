...
test.describe("Auth flow", () => {
  ...
  test('wrong passphrase shows "auth failed" and input is re-usable', async ({
    page,
  }) => {
    await page.goto("/");
    const passInput = page.locator("#pass");
  ...
  test('invalid saved passphrase shows "saved passphrase failed" and input is usable', async ({
    page,
  }) => {
    ...
    const status = page.locator("#status");
    await expect(status).toContainText("saved passphrase failed", {
      timeout: 10_000,
    });
