...
impl NvencDirectEncoder {
    ...
    pub fn try_new(codec: &str, width: u32, height: u32) -> Result<Self, String> {
        ...
        eprintln!("[nvenc-direct] initialized {codec} encoder for {width}x{height}");
    ...
    fn write_input_bgra_padded(&mut self, bgra: &[u8], src_w: usize, src_h: usize) -> Option<()> {
        let mut lock_buf = vec![0u8; NVENC_LOCK_INPUT_BUFFER_SIZE];
        w32(&mut lock_buf, 0, nvencapi_struct_version(1));
        wptr(&mut lock_buf, 8, self.input_buffer);
        ...
        let dst = rptr(&lock_buf, 16) as *mut u8;
        let pitch = r32(&lock_buf, 24) as usize;
        ...
        unsafe {
            for row in 0..enc_h {
                ...
                let src_row_start = sr * src_w * 4;
                // Copy source pixels
                let copy_w = enc_w.min(src_w);
                ptr::copy_nonoverlapping(bgra.as_ptr().add(src_row_start), dst_row, copy_w * 4);
                // Pad extra columns by duplicating the last source pixel
                if enc_w > src_w {
                    let last_px = bgra.as_ptr().add(src_row_start + (src_w - 1) * 4);
                    for col in src_w..enc_w {
                        ptr::copy_nonoverlapping(last_px, dst_row.add(col * 4), 4);
                    }
                }
            }
        }
    ...
    fn drop(&mut self) {
        unsafe {
            ...
            if let Some(cuda) = gpu_libs::cuda() {
                (cuda.cuCtxDestroy_v2)(self._cuda_ctx);
            }
        }
    }
}
