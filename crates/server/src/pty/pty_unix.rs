...
impl ExecPlan {
    /// `argv0_override` is what the child should see as its own name. Callers
    /// pass the program as the client wrote it, so `ps` and busybox-style
    /// dispatch see the request rather than the resolved path.
