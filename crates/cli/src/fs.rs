...
// ── One-shot queries: grep / cat / find ────────────────────────────────────
//
// These three need no sync: the root is an absolute server path on the
// request, so there is no `FS_SYNC` handshake and no sync_id. Connect, send
// one frame, read the answer that echoes our nonce.

/// Handshake, then refuse early if the server predates the fs family. A
/// server that lacks it drops the opcode silently, so without this the
...
async fn require_fs(
    ...
    if features & FEATURE_FS == 0 {
        return Err(
            "server does not support filesystem access (upgrade blit on the remote)".into(),
        );
    }
    Ok(())
}
...
pub async fn cmd_grep(
    ...
    let data = await_reply(&mut reader, &mut fb, S2C_FS_GREP, REQ_NONCE).await?;
    let Some((_, status, rflags, detail, records)) = parse_fs_grep_result(&data) else {
        return Err("malformed grep response".into());
    };
    if status != FS_DONE_OK {
        // `detail` carries the engine's own wording for a bad pattern,
        // which is the only useful thing to print.
        let why = if detail.is_empty() {
            fs_done_status_text(status).to_string()
        } else {
            detail
        };
        return Err(format!("search failed: {why}"));
    }

    let truncated = rflags & FS_GREP_TRUNCATED != 0;
    let mut path = String::new();
    let mut ignored = false;
    let mut hits = 0usize;
    for rec in fs_grep_records(&records) {
        match rec {
            FsGrepRecord::File { path: p, flags, .. } => {
                path = p;
                ignored = flags & FS_GREP_FILE_IGNORED != 0;
            ...
            FsGrepRecord::Match {
                ...
                if json {
                    ...
                    // grep(1)'s shape: PATH:LINE:TEXT, so editors and
                    // `cut -d:` pipelines already understand it.
                    println!("{path}:{}:{text}", line + 1);
                }
            }
        }
    }
    if truncated {
        eprintln!("blit: results truncated — a budget was reached");
    }
    // grep(1)'s convention: 1 means "no matches", which scripts test for.
    Ok(if hits == 0 { 1 } else { 0 })
}
...
pub async fn cmd_cat(transport: Transport, path: String, root: String) -> Result<i32, String> {
    ...
    let data = await_reply(&mut reader, &mut fb, S2C_FS_FILE, REQ_NONCE).await?;
    let Some((_, status, content)) = parse_fs_file(&data) else {
        return Err("malformed fetch response".into());
    };
    if status != FS_FILE_OK {
        let why = match status {
            FS_FILE_NOT_FOUND => "no such file",
            FS_FILE_UNREADABLE => "permission denied",
            _ => "unreadable",
        };
        return Err(format!("cannot read {path}: {why}"));
    }
    // Bytes straight through: this is `cat`, so it must not mangle a binary.
    std::io::stdout()
        .write_all(&content)
...
pub async fn cmd_find(
    ...
    let data = await_reply(&mut reader, &mut fb, S2C_FS_SEARCH, REQ_NONCE).await?;
    let Some((_, status, paths)) = parse_fs_search_result(&data) else {
        return Err("malformed search response".into());
    };
    if status != FS_STATUS_OK {
        return Err(format!("search failed: status {status}"));
    }
    for p in &paths {
        if json {
            println!("{}", serde_json::json!({"path": p}));
        } else {
            println!("{p}");
        }
    }
