...
use blit_remote::lsp::{
    ...
    LSP_QUERY_RENAME, LSP_QUERY_WS_SYMBOLS, LSP_REFS_INCLUDE_DECLARATION, LSP_RESP_INCOMPLETE,
    LSP_RESP_TRUNCATED, LSP_SIGNATURE_NO_PARAM, LSP_STATUS_NOT_FOUND, LSP_STATUS_OK,
...
/// LSP `CompletionItemKind` → a short tag. The names are the spec's,
/// lowercased; 0 and anything unknown read as "?".
fn completion_kind_name(kind: u8) -> &'static str {
    match kind {
        1 => "text",
        2 => "method",
        3 => "fn",
...
pub async fn cmd_position(
    ...
    for record in lsp_query_records(&records) {
        match &record {
            ...
            // The CLI never issues completion/signature queries (the
            // browser editor is their consumer; an agent's completion
            // engine is the LLM, docs/design/lsp.md).
            LspQueryRecord::Completion {
                ...
                if json {
                    println!(
                        ...
                        serde_json::json!({
                            "label": label,
                            "kind": completion_kind_name(*item_kind),
                            "insert": insert,
                            "detail": detail,
                        })
                    );
                } else {
                    // TSV: LABEL, KIND, DETAIL — greppable, and `cut -f1`
                    // gives a plain candidate list.
                    println!("{label}\t{}\t{detail}", completion_kind_name(*item_kind));
                }
                found += 1;
            }
            LspQueryRecord::Signature {
                ...
                if json {
                    println!(
                        ...
                        serde_json::json!({
                            "label": label,
                            "activeParam": if *active_param == LSP_SIGNATURE_NO_PARAM {
                                serde_json::Value::Null
                            } else {
                                serde_json::json!(active_param)
                            },
                    ...
                    println!("{label}");
                    // Underline the active parameter's span, so which
                    // argument you are on is visible without counting
                    // commas.
                    if *active_param != LSP_SIGNATURE_NO_PARAM
                        && *param_end as usize <= label.len()
                        && param_start < param_end
                    {
                        let pad = label[..*param_start as usize].chars().count();
                        let width = label[*param_start as usize..*param_end as usize]
                            .chars()
                            .count();
                        println!("{}{}", " ".repeat(pad), "~".repeat(width.max(1)));
                    }
                    if !doc.is_empty() {
                        println!("{doc}");
                    }
    ...
    // A RENAME whose WorkspaceEdit carried whole-file create/rename/delete
    // operations comes back with only the text EDIT records; the plan is
    // advisory, not a complete edit set (docs/design/lsp.md LSP_QUERY).
    if kind == LSP_QUERY_RENAME && resp_flags & LSP_RESP_INCOMPLETE != 0 {
        if json {
            println!("{}", serde_json::json!({ "type": "incomplete" }));
        } else {
            eprintln!(
                "… rename incomplete: whole-file create/rename/delete operations were dropped; the printed edits alone leave the workspace inconsistent"
            );
        }
    }
...
pub use blit_remote::lsp::{
    LSP_QUERY_COMPLETION as KIND_COMPLETE, LSP_QUERY_DEFINITION as KIND_DEF,
    LSP_QUERY_HOVER as KIND_HOVER, LSP_QUERY_REFERENCES as KIND_REFS,
    LSP_QUERY_RENAME as KIND_RENAME, LSP_QUERY_SIGNATURE as KIND_SIGNATURE,
};
